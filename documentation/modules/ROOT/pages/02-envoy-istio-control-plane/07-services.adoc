...
TIP: Follow {istio_doc}/tasks/traffic-management/ingress/secure-ingress/[Secure Ingress,window=_blank] for more information about generate certificates

[#addcert]
=== Create in Istio the Custom Certificate using SDS

First of all, we make a backup of the *front-javascript* gateway object. This is important because we will modify this object in order to add the custom certificate.

[.console-input]
[source,input,subs="+macros,+attributes"]
----
oc get gateway front-javascript -n <user_namespace> -o yaml > gw-front-javascript.yaml
----
