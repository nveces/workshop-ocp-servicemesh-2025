...
- *Gateways* - Describes a load balancer operating at the edge of the mesh receiving incoming or outgoing HTTP/TCP connections (Please, visit this link {istio_doc}/concepts/traffic-management/#gateways[Gateways,window=_blank] for more information about _gateways_)
- *Virtual Services* - Defines a set of traffic routing rules to apply when a host is addressed (Please, visit this link {istio_doc}/concepts/traffic-management/#virtual-services[Virtual Services,window=_blank] for more information about _virtual services_)
- *Destination Rules* - Defines policies that apply to traffic intended for a service after routing has occurred (Please, visit this link {istio_doc}/concepts/traffic-management/#destination-rules[Destination Rules,window=_blank] for more information about _destination rules_)
...
You can include the namespace yourself in the mesh by creating the ServiceMeshMember (SMM) object:

[.console-input]
[source,input,subs="+macros,+attributes"]
----
oc process -f 06-istio-ctrlplane/00-jump-app-ns-smr.yaml --param-file=params.env --ignore-unknown-parameters | oc apply -f -
----

Once the namespace has been included in the mesh, a series of network policies are created that we will examine later and you are no longer able to access to _Jump App_.
...
TIP: If you are an advanced user and can not wait for the instructor, please visit xref:02-envoy-istio-control-plane/index.adoc#smr[Create ServiceMeshMember Object] and follow the instructions included there and go back again. Anyway, we will return to this point in that section xref:02-envoy-istio-control-plane/index.adoc#smr[Create ServiceMeshMember Object].
...
.Back Golang Patched
image::02-envoy-istio-control-plane/jump-app-back-go-depl-patch.png[link=../_images/02-envoy-istio-control-plane/jump-app-back-go-depl-patch.png,window=_blank]
...
[source,input,subs="+macros,+attributes"]
----
oc process -f 03-jump-app-flows/04-jump-app-routes.yaml --param-file=params.env --ignore-unknown-parameters | oc apply -n istio-system -f -
----
...
Jump App is generating traffic flow thanks to the frontend where was defined an specific number of continuous jumps. In order to review the Service Mesh traffic flow in your project, please visit Kiali console *<kiali_url>* (_E.g. {kiali_url}[window=_blank]_):
