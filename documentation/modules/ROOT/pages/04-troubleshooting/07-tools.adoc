...
First of all, to use *Jaeger* in your *Control plane* you need to have it installed in your cluster.
You can install *Red Hat OpenShift distributed tracing platform Operator* from the OperatorHub in the OpenShift console. Once you have it installed.

image::04-troubleshooting/tracing-jaeger-03.png[link=../_images/04-troubleshooting/tracing-jaeger-03.png,window=_blank]

image::04-troubleshooting/tracing-jaeger-04.png[link=../_images/04-troubleshooting/tracing-jaeger-04.png,window=_blank]



, you can create a new instance of Jaeger by going to the *Jaeger* section in the left menu and clicking on `Create Instance`. You can use the default values for the installation.
...
<1> Change `None` to `Jaeger` to enable *Jaeger* in the *Service Mesh Control Plane*.
<2> Configure addon to create a *Jaeger* instance using Memory storage.


image::04-troubleshooting/tracing-jaeger-05.png[link=../_images/04-troubleshooting/tracing-jaeger-05.png,window=_blank]

image::04-troubleshooting/tracing-jaeger-06.png[link=../_images/04-troubleshooting/tracing-jaeger-06.png,window=_blank]


Click on link to access the *Jaeger UI*:

[.console-input]
image::04-troubleshooting/tracing-jaeger-07.png[link=../_images/04-troubleshooting/tracing-jaeger-07.png,window=_blank]
...
To know more about all the statistics, you could check the link:https://www.envoyproxy.io/docs/envoy/latest/configuration/upstream/cluster_manager/cluster_stats#statistics[Envoy docs,window=_blank].
...
Envoy has several endpoints that you can use to check the status of the proxy. You can see all of them by executing the following command:
...
curl -v http://localhost:15000/certs
curl -v http://localhost:15000/clusters
curl -v http://localhost:15000/contention
curl -v http://localhost:15000/heap_dump
curl -v http://localhost:15000/help
curl -v http://localhost:15000/hot_restart_version
curl -v http://localhost:15000/memory
curl -v http://localhost:15000/ready
curl -v http://localhost:15000/runtime
curl -v http://localhost:15000/server_info
curl -v http://localhost:15000/stats_recentlookups
----
...
[source]
----
Namespace -: openshif-ingress
Pod -------: router-<router-name>
Container -: router
...
Running on: Linux 5.14.0-427.62.1.el9_4.x86_64 #1 SMP PREEMPT_DYNAMIC Sat Mar 22 10:20:42 EDT 2025 x86_64
----

* Configuration file: `/var/lib/haproxy/conf/haproxy.config`
* Executable file: `/usr/sbin/haproxy`
* PID file: `/var/lib/haproxy/run/haproxy.pid`
...
Pod -------: istio-ingressgateway-<pod-name>
Container -: istio-proxy
Image -----: registry.redhat.io/openshift-service-mesh/proxyv2-rhel9 <1>
Product ---: Envoy proxy
----

Where:

<1> https://catalog.redhat.com/software/containers/openshift-service-mesh/proxyv2-rhel9/64010fd529d6b9c7d9c6bdc2[window=_blank]
...
[source,input,subs="+macros,+attributes"]
----
sh-5.1$ cd /usr/local/bin/
sh-5.1$ ./envoy --version
./envoy  version: c6160ecc199f34ab6c3e6a5f16958d150e536faa/1.28.8-dev/OSSM 2.6.7-1/RELEASE/OpenSSL
----

*Istio Pilot agent* runs in the sidecar or gateway container and bootstraps Envoy.

.Istio Pilot agent version.
[.console-input]
[source,input,subs="+macros,+attributes"]
----
sh-5.1$ ./pilot-agent version
version.BuildInfo{Version:"OSSM_2.6.7-1", GitRevision:"e1d197c84f9e1f436139bcdf81215d67a8edfd81", GolangVersion:"go1.21.13 (Red Hat 1.21.13-5.el9_4) X:strictfipsruntime", BuildStatus:"Clean", GitTag:"2.6.7"}
----
