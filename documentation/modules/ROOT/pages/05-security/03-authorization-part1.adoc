...
- The {istio_doc}/reference/config/networking/sidecar/#WorkloadSelector[selector,window=_blank] field specifies the target of the policy.
- The {istio_doc}/reference/config/security/authorization-policy/#AuthorizationPolicy-Action[action,window=_blank] field specifies whether to allow or deny the request.
- The {istio_doc}/reference/config/security/authorization-policy/#Rule[rules,window=_blank] specify when to trigger the action
...
We will be creating this kind of manifest during the following labs, so in case you have any doubt, please feel free to ask. I also recommend to have a look at the {istio_doc}/reference/config/security/authorization-policy/[official documentation,window=_blank].
...
As per {istio_doc}/reference/config/security/authorization-policy/#AuthorizationPolicy[doc,window=_blank], the above means:
...
image::05-security/kiali-rbac-error.png[Kiali RBAC error,link=../_images/05-security/kiali-rbac-error.png,window=_blank]
...
The answer is that in the Kubernetes world, {istio_doc}/concepts/security/#istio-identity[Istio uses an identity,window=_blank] associated to each `ServiceAccount`. Remember that we must have mTLS enabled, so for each caller service, there will be a client certificate present.

To better understand it, we will set the `rbac` log level to debug in our `productpage` pod and determine what is the value coming in the `source.principal` attribute (which is the attribute that is used to be checked as principal, according to the {istio_doc}/reference/config/security/authorization-policy/#Source[docs,window=_blank])
...
....
$ oc process -f $LABS_HOME/lab3/allow-ingress-productpage.yaml -p INGRESS_GW=istio-ingressgateway -o yaml -n $APPS_NS1 | oc apply -f - -n $APPS_NS1
...
image::05-security/kiali-rbac-ok.png[Kiali RBAC ok,link=../_images/05-security/kiali-rbac-ok.png,window=_blank]
...
image::05-security/productpage-accesing-ns2.png[RBAC error ns2,link=../_images/05-security/productpage-accesing-ns2.png,window=_blank]
