...
....
$ oc process -f $LABS_HOME/lab1/istio/productpage-v1.yaml -p PRODUCTPAGE_HOST=$PRODUCTPAGE_HOST -p INGRESS_GW=$USER -p APPS_NS=$APPS_NS1 -p MESH_NS=$SERVICE_MESH_NS -o yaml | oc apply -f -
...
....
$ oc get route/${APPS_NS1}-productpage -o template='http://{{.spec.host}}' -n istio-system
...
image::05-security/first-graph.png[First graph,link=../_images/05-security/first-graph.png,window=_blank]
...
image::05-security/graph-with-ns2.png[Graph with ns2,link=../_images/05-security/graph-with-ns2.png,window=_blank]
