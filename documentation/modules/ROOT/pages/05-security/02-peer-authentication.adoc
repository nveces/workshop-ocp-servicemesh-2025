...
To do so, we will create a {istio_doc}/reference/config/security/peer_authentication/[PeerAuthentication,window=_blank] object that set the mTLS mode to `strict`:
...
image::05-security/security-check.png[Mtls in ns1,link=../_images/05-security/security-check.png,window=_blank]
