...
:email: splatas@redhat.com
:imagesdir: ./images
:toc: left
:toc-title: Authorization: part 2

[Abstract]
In this last lab, we will see the use of a token (JWT) in order to allow or block the consume of a service.
...
// ....
// export MY_CLIENT="391916373246-6sl1ld3qoqv6k1atnrbli75vpq7bsfrb.apps.googleusercontent.com"
// export MY_SECRET="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"
// export GOOGLE_OIDC_ISSUER_URL=https://accounts.google.com


// $ oc process -f $LABS_HOME/lab5/oauth2-proxy.yaml \
//     -p USER=$USER \
//     -p INGRESS_GW=$INGRESS_GW \
//     -p APP_DOMAIN=$PRODUCTPAGE_HOST \
//     -p CLIENT_ID=$MY_CLIENT \
//     -p CLIENT_SECRET=$MY_SECRET \
//     -p COOKIE_SECRET=$(openssl rand -hex 16) \
//     -p OIDC_ISSUER_URL=$GOOGLE_OIDC_ISSUER_URL \
//     -p OAUTH_NS=$APPS_NS1 -n $APPS_NS1 | oc apply -f - -n $APPS_NS1
...
// NOTE: There were some basic objects created that you are familiar to as in any regular OCP+Mesh application deployment: `Service`, `Deployment`, `VirtualService` and `DestinationRule`. Then, there are two `ServiceEntries` that have been created in order to allow access to external resources which are Google's related. This is because we have restricted the access to any external resource from within the `Service Mesh` (as we are in the Security Workshop, we would like to have certain control to which external resources are consumed). To achieve this goal, we configured our `ServiceMeshControlPlane` with a `REGISTRY_ONLY` `outboutTrafficPolicy` {istio_doc}/reference/config/istio.mesh.v1alpha1/[mode,window=_blank]. Feel free to have a look at the configuration and the `ServiceEntries` as well.
...
// [2021/04/03 20:49:58] [oauthproxy.go:137] OAuthProxy configured for OpenID Connect Client ID: 80.....apps.googl....ontent.com
...
*Reference*: https://istio.io/latest/docs/tasks/security/authorization/authz-jwt/

=== Before you begin this task, do the following:
- Complete the Istio end user authentication task. Ref: https://istio.io/latest/docs/tasks/security/authentication/authn-policy/#end-user-authentication

- Read the Istio authorization concepts. Ref: https://istio.io/latest/docs/concepts/security/#authorization

- Install Istio using Istio installation guide. Ref: https://istio.io/latest/docs/setup/install/istioctl/
...
$ cd lab-05/lab6/
```
Remember that our repository of reference is: https://github.com/nveces/ocp-servicemesh-labs-2025.git
...
```
$ oc project $MY_PROJECT ('user01' in our example)
...
*Reference*: https://istio.io/latest/docs/tasks/security/authorization/authz-jwt/#allow-requests-with-valid-jwt-and-list-typed-claims

The following command creates the *'jwt-example' request authentication* policy for the httpbin workload in the namespace. This policy for httpbin workload accepts a JWT issued by testing@secure.istio.io:
```
...
Get the JWT that sets the iss and sub keys to the same value, testing@secure.istio.io. This causes Istio to generate the attribute requestPrincipal with the value testing@secure.istio.io/testing@secure.istio.io:
```
...
The following command **updates** the require-jwt authorization policy to also require the **JWT to have a claim named groups** containing the value **group1**:
```
