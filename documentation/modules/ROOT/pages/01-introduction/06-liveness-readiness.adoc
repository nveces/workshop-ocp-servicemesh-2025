...
To answers these two questions, what are liveness/readiness probes? why are they needed? read the next article (10 minutes) that will give through different examples and scenarios the response to both questions: https://developers.redhat.com/blog/2020/11/10/you-probably-need-liveness-and-readiness-probes/[window=_blank]

== Configure liveness and readiness

=== Application endpoints

Our application, which is based on Quarkus, provides a built-in health check endpoints when including `quarkus-smallrye-health` extension (for more detailes, see how it is done in our example application https://github.com/dsanchor/petclinic/blob/mnl-config/quarkus-petclinic/pom.xml#L73[here,window=_blank]).
...
image::01-introduction/add-hc.png[add hc]
...
....
oc scale deployment/petclinic-database --replicas=0 -n <YOUR_PROJECT>

deployment.apps/petclinic-database scaled
....
...
....
oc scale deployment/petclinic-database --replicas=1 -n <YOUR_PROJECT>
...
Everything should be back to normal: try again with "curl -v http://<APP_HOST_PORT>/health/ready".
