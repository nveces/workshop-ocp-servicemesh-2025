...
I would like you to spend some time to read the Openshift official documentation about storage and understand the main concepts around it: https://docs.openshift.com/container-platform/4.6/storage/understanding-persistent-storage.html[Understanding persistent storage,window=_blank].

Or in case you just want the basics for the lab, just read the following:

- https://docs.openshift.com/container-platform/4.6/storage/understanding-persistent-storage.html#persistent-volumes_understanding-persistent-storage[PersistentVolume,window=_blank]
- https://docs.openshift.com/container-platform/4.6/storage/understanding-persistent-storage.html#persistent-volumes_understanding-persistent-storage[PersistentVolumeClaim,window=_blank]
...
....
$ oc scale deployment/postgresql --replicas=0 -n <YOUR_PROJECT>
...
Scale the Database `Deployment` back to one replica:

....
$ oc scale deployment/postgresql --replicas=1 -n <YOUR_PROJECT>
...
- Have a look at the new `Deployment` volume and volumeMount section
